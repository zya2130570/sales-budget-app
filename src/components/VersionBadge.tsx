...
function ChangelogModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70" onClick={onClose}>
      <div className="w-full max-w-2xl max-h-[85vh] flex flex-col rounded-2xl border border-slate-700 bg-slate-800 shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700 flex-shrink-0">
          <div>
            <h2 className="text-sm font-semibold text-slate-100">Version changelog</h2>
            <p className="text-xs text-slate-500 mt-0.5">What changed · What to click to verify</p>
          </div>
          <button onClick={onClose} className="text-slate-500 hover:text-slate-300 text-lg transition-colors">✕</button>
        </div>
        <div className="flex flex-1 min-h-0">
          <div className="w-24 flex-shrink-0 border-r border-slate-700 py-2 overflow-y-auto">
            {CHANGELOG.map((entry, i) => (
              <button key={entry.version} onClick={() => setSelected(entry)}
                className={`w-full text-left px-3 py-2.5 transition-colors ${selected.version === entry.version ? 'bg-blue-600/20 text-blue-300 border-r-2 border-blue-500' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-700/50'}`}>
              ...
              <ul className="space-y-2">
                {selected.test.map((item, i) => (
                  <li key={i} className="flex gap-2 text-xs text-emerald-200/80">
                    <span className="text-emerald-500 flex-shrink-0 font-bold mt-0.5">{i + 1}.</span>{item}
                  </li>
                ))}
              </ul>
            </div>
