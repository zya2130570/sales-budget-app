/**
 * KeyboardShortcutsPanel.tsx — V40
 * Editable keyboard shortcuts reference. Built-in shortcuts show their key and
 * description; users can edit descriptions and add/delete custom reference rows.
 * Custom rows are persisted to localStorage. Built-in keys cannot be rebound
 * (they are hardcoded in Sidebar.tsx) but their display labels are editable.
 */
...
export function KeyboardShortcutsPanel({ onClose }: Props) {
  ...
  function addRow() {
    const k = addKeys.trim()
    const a = addAction.trim()
    if (!k || !a) return
    setCustom(prev => [...prev, { id: `custom-${Date.now()}`, keys: k, action: a, builtIn: false }])
    setAddKeys('')
    setAddAction('')
    setShowAdd(false)
  }
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-lg rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl flex flex-col max-h-[85vh]"
            ...
            editingId === row.id ? (
              <div key={row.id} className="rounded-xl border border-blue-600/60 bg-blue-950/30 px-3 py-2.5 space-y-2">
                <div className="flex gap-2">
                  <div className="flex-shrink-0 w-28">
                    ...
                    <input
                      autoFocus
                      className="w-full rounded-md border border-slate-600 bg-slate-800 px-2 py-1 font-mono text-xs text-slate-200 focus:outline-none focus:border-blue-500"
                      value={editKeys}
                  ...
                  <div className="flex-1">
                    ...
                    <input
                      className="w-full rounded-md border border-slate-600 bg-slate-800 px-2 py-1 text-xs text-slate-200 focus:outline-none focus:border-blue-500"
                      value={editAction}
              ...
              >
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  {!row.builtIn && (
                    <button
                      onClick={e => { e.stopPropagation(); deleteCustom(row.id) }}
          ...
          {showAdd ? (
            <div className="rounded-xl border border-green-700/50 bg-green-950/20 px-3 py-2.5 space-y-2">
              ...
              <div className="flex gap-2 justify-end">
                ...
                <button onClick={addRow} disabled={!addKeys.trim() || !addAction.trim()} className="text-xs text-green-400 hover:text-green-200 px-2 py-1 rounded bg-green-900/40 hover:bg-green-900/70 disabled:opacity-40 disabled:cursor-not-allowed">Add</button>
              </div>
            </div>
          ) : (
            <button
            ...
            >
              + Add custom shortcut reference
            </button>
          )}
        </div>

        {/* Footer */}
        <div className="border-t border-slate-700/60 px-5 py-3 flex-shrink-0">
          <p className="text-xs leading-relaxed text-slate-500">
            Shortcuts pause automatically inside text fields and dropdowns. Built-in key bindings cannot be rebound here — edit descriptions only.
          </p>
        </div>
      </div>
    </div>
