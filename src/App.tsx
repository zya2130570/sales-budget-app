...
const remainingToneFromValues = (remaining: number, totalNetIncome: number): 'good' | 'warn' | 'danger' => {
  if (remaining < 0 || totalNetIncome <= 0) return 'danger'
  const cushion = remaining / totalNetIncome
  if (cushion >= 0.2) return 'good'
  if (cushion >= 0.05) return 'warn'
  return 'danger'
}
...
export default function App() {
  ...
  const remainingTone = remainingToneFromValues(selectedPeriodRemaining, selectedPeriodTotalNet)
