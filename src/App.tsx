  ...
  <section className="space-y-4 transition-all duration-300">
    <Card title="Income Input">
      ...
      <div className="relative mt-2">
        ...
        <input
          ...
          value={gpInput}
          onChange={(e) =>
            setGpInput(String(Math.max(0, Number(e.target.value) || 0)))
          }
      ...
      <div className="mt-2 text-xs text-slate-400">
        ...
        <span className="font-semibold text-slate-200">
          {currency(adjustedSalary)}
        </span>
      </div>

      <p className="text-xs text-slate-400 mt-1">
        {typeof nextThreshold !== 'undefined'
          ? `Next base bump at ${currency(nextThreshold)}`
          : 'All base bumps achieved'}
      </p>
      ...
      {typeof nextThreshold !== 'undefined' && gp >= nextThreshold && (
        <label className="mt-2 inline-flex items-center gap-2 text-xs text-slate-300">
          <input
            ...
            onChange={(e) => {
              if (e.target.checked) {
                setBaseBumpsAchieved((prev) => prev + 1)
              }
            }}
          />
          Base bump achieved
    ...
    <div className="grid md:grid-cols-2 gap-4">
      ...
      <Card title="Efficiency Metrics">
        <Row
          l="Effective hourly net rate"
          v={currency(inc.totalWeekly / HOURS_PER_WEEK) + ' per hour'}
        />
        <Row l="Commission as % of total" v={`${dep.toFixed(1)}%`} />
        <Row
          l="Commission per hour"
          v={currency(inc.cWeekly / HOURS_PER_WEEK)}
        />
      </Card>
    </div>
  </section>
