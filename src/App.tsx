...
export default function App() {
  ...
  return <div className="min-h-screen bg-slate-900 text-slate-100"><div className="max-w-7xl mx-auto p-4 md:p-8 space-y-6">
    ...
    {tab === 'Targets' && <section className="space-y-4"><Card title="Create Target"><div className="grid md:grid-cols-5 gap-2"><div className="relative"><input className="w-full p-2 rounded bg-slate-800 border border-slate-600" value={targetForm.name} onFocus={()=>setShowTargetSuggestions(true)} onChange={(e)=>{setTargetForm((v)=>({...v,name:e.target.value})); setTargetSuggestionIndex(-1); setShowTargetSuggestions(true)}} onKeyDown={(e)=>{ if(!targetSuggestionList.length) return; if(e.key==='ArrowDown'){e.preventDefault(); setTargetSuggestionIndex((v)=>Math.min(v+1,targetSuggestionList.length-1))} if(e.key==='ArrowUp'){e.preventDefault(); setTargetSuggestionIndex((v)=>Math.max(v-1,0))} if(e.key==='Enter'){e.preventDefault(); if(targetSuggestionIndex>=0){setTargetForm((v)=>({...v,name:targetSuggestionList[targetSuggestionIndex]})); setShowTargetSuggestions(false)}} }} placeholder="Target Name" />{showTargetSuggestions && targetSuggestionList.length>0 && <div className="absolute z-10 w-full mt-1 max-h-48 overflow-y-auto bg-slate-800 border border-slate-600 rounded-lg">{targetSuggestionList.map((p,i)=><button key={p} className={`w-full text-left px-2 py-1 ${i===targetSuggestionIndex?'bg-slate-700':'hover:bg-slate-700'}`} onClick={()=>{setTargetForm((v)=>({...v,name:p})); setShowTargetSuggestions(false)}}>{p}</button>)}</div>}</div><input type="number" min={0} step={25} className="p-2 rounded bg-slate-800 border border-slate-600" value={targetForm.goalAmount} onChange={(e)=>setTargetForm((v)=>({...v,goalAmount:e.target.value}))} placeholder="Goal Amount" /><input type="number" min={0} step={25} className="p-2 rounded bg-slate-800 border border-slate-600" value={targetForm.currentSaved} onChange={(e)=>setTargetForm((v)=>({...v,currentSaved:e.target.value}))} placeholder="Current Saved" /><input type="date" className="p-2 rounded bg-slate-800 border border-slate-600" value={targetForm.deadline} onChange={(e)=>setTargetForm((v)=>({...v,deadline:e.target.value}))} /><button className="rounded bg-blue-600" onClick={()=>{ if(!targetForm.name || !targetForm.goalAmount || !targetForm.deadline) return; setTargets((p)=>[{ id: crypto.randomUUID(), name: targetForm.name, goalAmount: Number(targetForm.goalAmount), currentSaved: Number(targetForm.currentSaved)||0, deadline: targetForm.deadline, type:'savings', contributions: [] }, ...p]); setTargetForm({ name:'Bike', goalAmount:'', currentSaved:'0', deadline:'' }) }}>Create</button></div></Card>
