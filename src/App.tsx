...
function Metric({ title, value, tone = 'neutral', featured = false }: { title: string; value: string; tone?: 'neutral'|'good'|'warn'|'danger'; featured?: boolean }) { const c=tone==='good'?'text-green-400':tone==='warn'?'text-yellow-300':tone==='danger'?'text-red-400':'text-slate-100'; return <div className={`rounded-xl border p-3 ${featured ? 'border-green-400/70 bg-slate-700/70 shadow-[0_0_18px_rgba(74,222,128,0.25)]' : 'border-slate-700 bg-slate-800'}`}><div className="text-xs text-slate-400 mb-1">{title}</div><div className={`${featured ? 'text-2xl' : 'text-xl'} font-bold ${c}`}>{value}</div></div> }
