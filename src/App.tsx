...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (tab === 'Budget') budgetNameRef.current?.focus()
  ...
  const depColor = dep <= 30 ? 'text-green-400' : dep <= 55 ? 'text-yellow-300' : 'text-red-400'

  const top = [...categories].sort((a, b) => b.amount - a.amount)
  const suggestionList = form.name ? categorySuggestions.filter(s => s.toLowerCase().includes(form.name.toLowerCase())).slice(0, 6) : []

  const health = monthlyLeft < 0 || monthlyLeft / inc.totalMonthly < 0.05 || inc.baseMonthly < monthlyBudget ? 'Risk' : monthlyLeft / inc.totalMonthly >= 0.2 && savingsRate >= 15 && fixedRatio <= 50 && dep <= 50 ? 'Strong' : monthlyLeft / inc.totalMonthly >= 0.1 && savingsRate >= 10 ? 'Stable' : 'Tight'
  const welcome = health === 'Strong' ? `You’re in a strong position. Savings and investing are healthy, and fixed bills are under control.` : health === 'Stable' ? `Your current plan is stable. You have ${currency(monthlyLeft)} remaining this month after planned spending, and commission makes up ${dep.toFixed(0)}% of your total income.` : `Your budget is tight. Remaining cushion is low and this plan depends heavily on commission.`
  ...
  return <div className="min-h-screen bg-slate-900 text-slate-100"><div className="max-w-7xl mx-auto p-4 md:p-8 space-y-6">
    ...
    {tab === 'Budget' && <section className="space-y-4 transition-all duration-300"><Card title="Budget Summary"><div className="flex gap-2 flex-wrap mb-4">{periods.map(p => <Pill key={p} active={bp===p} onClick={() => setBp(p)}>{labelPeriod(p)}</Pill>)}</div><div className="grid md:grid-cols-4 gap-3"><Metric title="Total available income" value={currency(convertFromMonthly(inc.totalMonthly,bp))} /><Metric title="Total planned expenses" value={currency(convertFromMonthly(monthlyBudget,bp))} /><Metric title="Remaining amount" value={currency(convertFromMonthly(monthlyLeft,bp))} /><Metric title="Budget status" value={health} /></div></Card>
    <Card title="Budget Categories"><div className="grid md:grid-cols-4 gap-2"><div className="relative"><input ref={budgetNameRef} className="w-full p-2 rounded-lg bg-slate-800 border border-slate-600" placeholder="Category name" value={form.name} onChange={e => { setForm(v=>({...v,name:e.target.value})); setSIndex(-1) }} onKeyDown={e => { if (!suggestionList.length) { if (e.key==='Enter') budgetAmountRef.current?.focus(); return } if (e.key==='ArrowDown') { e.preventDefault(); setSIndex(v => Math.min(v+1, suggestionList.length-1)) } if (e.key==='ArrowUp') { e.preventDefault(); setSIndex(v => Math.max(v-1,0)) } if (e.key==='Enter') { e.preventDefault(); if (sIndex>=0) { setForm(v=>({...v,name:suggestionList[sIndex]})); budgetAmountRef.current?.focus() } else budgetAmountRef.current?.focus() } }} />{suggestionList.length>0 && <div className="absolute z-10 w-full mt-1 bg-slate-800 border border-slate-600 rounded-lg">{suggestionList.map((x,i)=><button key={x} className={`w-full text-left px-2 py-1 ${i===sIndex?'bg-slate-700':''}`} onClick={()=>{setForm(v=>({...v,name:x})); budgetAmountRef.current?.focus()}}>{x}</button>)}</div>}</div><input ref={budgetAmountRef} type="number" min={0} step={25} placeholder={`${labelPeriod(bp)} Amount`} className="p-2 rounded-lg bg-slate-800 border border-slate-600" value={form.amount} onChange={e=>setForm(v=>({...v,amount:e.target.value}))} onKeyDown={e=>{if(e.key==='Enter') budgetTypeRef.current?.focus()}} /><select ref={budgetTypeRef} className="p-2 rounded-lg bg-slate-800 border border-slate-600" value={form.type} onKeyDown={e=>{ if(['1','2','3','4'].includes(e.key)){ const m={'1':'fixed bill','2':'variable spending','3':'savings','4':'investing'} as const; setForm(v=>({...v,type:m[e.key as keyof typeof m]})) } if(e.key==='Enter'){e.preventDefault(); upsert()}}} onChange={e=>setForm(v=>({...v,type:e.target.value as CategoryType}))}><option value="fixed bill">1 - Fixed bill</option><option value="variable spending">2 - Variable spending</option><option value="savings">3 - Savings</option><option value="investing">4 - Investing</option></select><button onClick={upsert} className="rounded-lg bg-blue-600 hover:bg-blue-500">{editId?'Save Changes':'Add'}</button></div>
