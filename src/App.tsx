...
export default function App() {
  ...
  const welcome = !hasBudgetData ? 'No budget data yet. Add expenses to see your financial health.' : selectedPeriodRemaining < 0 ? `You are over budget by ${currency(Math.abs(selectedPeriodRemaining))} ${periodPhrase}. Reduce planned spending or increase income to get back above zero.` : health === 'Strong' ? `You’re in a strong position. Savings and investing are healthy, and fixed bills are under control.` : health === 'Stable' ? `Your current plan is stable. You have ${currency(monthlyLeft)} remaining ${periodPhrase} after planned spending, and commission makes up ${dep.toFixed(0)}% of your total income.` : `Your budget is tight. Remaining cushion is low and this plan depends heavily on commission.`
