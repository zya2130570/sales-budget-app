...
export default function App() {
  ...
  return <div className="min-h-screen bg-slate-900 text-slate-100"><div className="max-w-7xl mx-auto p-4 md:p-8 space-y-6">
    ...
    {tab === 'Budget' && <section className="space-y-4 transition-all duration-300"><Card title="Budget Summary"><div className="flex gap-2 flex-wrap mb-4">{periods.map(p => <Pill key={p} active={period===p} onClick={() => setPeriod(p)}>{labelPeriod(p)}</Pill>)}</div><div className="grid md:grid-cols-4 gap-3"><Metric title="Total available income" value={currency(convertFromMonthly(inc.totalMonthly,period))} /><Metric title="Total planned expenses" value={currency(convertFromMonthly(monthlyBudget,period))} /><Metric title="Remaining amount" value={currency(convertFromMonthly(monthlyLeft,period))} tone={remainingTone} /><Metric title="Budget status" value={health} tone={statusTone} /></div></Card>
...
function Metric({ title, value, tone = 'neutral', featured = false }: { title: string; value: string; tone?: 'neutral'|'good'|'warn'|'danger'; featured?: boolean }) { const c=tone==='good'?'text-green-400':tone==='warn'?'text-yellow-300':tone==='danger'?'text-red-400':'text-slate-100'; return <div className={`rounded-xl border p-3 ${featured ? 'border-slate-300/70 bg-slate-700/80 shadow-[0_0_18px_rgba(125,211,252,0.22)]' : 'border-slate-700 bg-slate-800'}`}><div className="text-xs text-slate-400 mb-1">{title}</div><div className={`${featured ? 'text-3xl text-slate-50' : `text-xl ${c}`} font-bold`}>{value}</div></div> }
