...
export default function App() {
  ...
  return <div className="min-h-screen bg-slate-900 text-slate-100"><div className="max-w-7xl mx-auto p-4 md:p-8 space-y-6">
    ...
    {tab === 'Targets' && <section className="space-y-4"><Card title="Create Target"><div className="grid md:grid-cols-5 gap-2"><input list="target-presets" className="p-2 rounded bg-slate-800 border border-slate-600" value={targetForm.name} onChange={(e)=>setTargetForm((v)=>({...v,name:e.target.value}))} placeholder="Target Name" /><datalist id="target-presets">{targetPresets.map(p=><option key={p} value={p} />)}</datalist><input type="number" min={0} step={25} className="p-2 rounded bg-slate-800 border border-slate-600" value={targetForm.goalAmount} onChange={(e)=>setTargetForm((v)=>({...v,goalAmount:e.target.value}))} placeholder="Goal Amount" /><input type="number" min={0} step={25} className="p-2 rounded bg-slate-800 border border-slate-600" value={targetForm.currentSaved} onChange={(e)=>setTargetForm((v)=>({...v,currentSaved:e.target.value}))} placeholder="Current Saved" /><input type="date" className="p-2 rounded bg-slate-800 border border-slate-600" value={targetForm.deadline} onChange={(e)=>setTargetForm((v)=>({...v,deadline:e.target.value}))} /><button className="rounded bg-blue-600" onClick={()=>{ if(!targetForm.name || !targetForm.goalAmount || !targetForm.deadline) return; setTargets((p)=>[{ id: crypto.randomUUID(), name: targetForm.name, goalAmount: Number(targetForm.goalAmount), currentSaved: Number(targetForm.currentSaved)||0, deadline: targetForm.deadline, type:'savings', contributions: [] }, ...p]); setTargetForm({ name:'Bike', goalAmount:'', currentSaved:'0', deadline:'' }) }}>Create</button></div></Card>
