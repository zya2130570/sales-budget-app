...
export default function App() {
  ...
  const statusLabel = hasBudgetData ? remainingTier.label : 'No Data'
...
function Metric({ title, value, tone = 'neutral', featured = false }: { title: string; value: string; tone?: 'neutral'|'good'|'warn'|'danger'; featured?: boolean }) { const c=tone==='good'?'text-green-400':tone==='warn'?'text-yellow-300':tone==='danger'?'text-red-400':'text-slate-100'; return <div className={`rounded-xl border p-3 ${featured ? 'border-sky-200/70 bg-gradient-to-br from-slate-700 via-slate-700/95 to-slate-600/95 shadow-[0_0_24px_rgba(125,211,252,0.28)]' : 'border-slate-700 bg-slate-800'}`}>{featured && <div className="mb-1 inline-flex rounded-full border border-sky-200/40 bg-slate-500/20 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-sky-100">Primary Take-Home</div>}<div className="text-xs text-slate-400 mb-1">{title}</div><div className={`${featured ? 'text-xl text-sky-100' : `text-xl ${c}`} font-bold`}>{value}</div></div> }
