...
type Target = { id: string; name: string; goalAmount: number; currentSaved: number; deadline: string; type: 'savings'; contributions: Contribution[]; showHistory?: boolean }
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (tab === 'Targets') { targetNameRef.current?.focus(); if (!targetForm.name.trim()) setShowTargetSuggestions(true) }
  }, [tab])
  useEffect(() => {
...
/>
		{showTargetSuggestions && targetSuggestionList.length>0 && <div className="absolute z-10 w-full mt-1 max-h-48 overflow-y-auto bg-slate-800 border border-slate-600 rounded-lg">{targetSuggestionList.map((preset,i)=><button key={preset} type="button" className={`w-full text-left px-2 py-1 ${i===targetSuggestionIndex?'bg-slate-700':'hover:bg-slate-700'}`} onMouseDown={(e)=>e.preventDefault()} onClick={()=>{setTargetForm((v)=>({...v,name:preset})); setShowTargetSuggestions(false); setTargetSuggestionIndex(-1); targetGoalRef.current?.focus()}}>{preset}</button>)}</div>}</div><input
  ref={targetGoalRef}
  type="number"
  min={0}
  step={25}
