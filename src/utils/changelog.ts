...
export const CHANGELOG: VersionEntry[] = [
  {
    version: 'V56',
    date: '6/12/2026 at 12:00 AM',
    what: [
      'Mobile sidebar redesign (Option A): on screens narrower than 640px, the sidebar now always shows icon + label stacked vertically, with each nav item getting a distinct color (Dashboard purple, Income teal, Budget blue, Accounts coral, Transactions amber, Scenarios pink, Savings Goals green). No more expanding/collapsing to see labels — everything is visible at all times.',
      'The collapse/expand toggle is hidden entirely on mobile since there\'s nothing to collapse anymore.',
      'Desktop sidebar (≥640px) is completely unchanged — same collapse/expand behavior, same icon style, same widths.',
      'Fixed a critical Supabase sync bug: import_batches table was missing a "skipped_count" column that the app has been sending on every CSV/PDF import sync since V49. Every import batch upsert was silently failing — your import_batches table had 0 rows despite many imports. Added the missing column via migration. New imports (and re-syncs of existing data) will now populate this table correctly.',
    ],
    test: [
      'On phone: open the app. Sidebar should show all 7 icons with labels underneath (Dashboard, Income, Budget, Accounts, Txns, Scenarios, Goals), each a different color. No need to tap anything to see labels.',
      'Tap a nav item — it should navigate immediately and show as active (colored left border + tinted background).',
      'On desktop: sidebar should look and behave exactly as before — collapse/expand still works, hover labels still work.',
      'Cloud: do a new CSV import, then check Cloud Status → Sync. After sync, the Transactions tab → Import History should now persist to cloud (previously it never did due to the column bug).',
    ],
  },
  {
    version: 'V43',
